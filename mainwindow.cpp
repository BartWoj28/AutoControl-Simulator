    ...
    poziomZaklocenia(0.0)
{
    ...
    //connect(ui->pushButtonARX, &QPushButton::clicked, this, &MainWindow::on_pushButtonARX_clicked);
...
void MainWindow::on_pushButtonARX_clicked() {
    ...
    if (dialog.exec() == QDialog::Accepted) {
        ...
        // JEŚLI OBIEKT UKLAD ISTNIEJE → AKTUALIZUJ JEGO PARAMETRY
        if (uklad) {
...
void MainWindow::on_comboBoxSposobCalkowania_currentIndexChanged(int index) {
    if (uklad) {
        // index 0: Ti poza całką (czyli podzielone przez Ti na końcu)
        // index 1: Ti pod całką (czyli wciągnięte do sumy)
        bool poza = (index == 0);
        uklad->getRegulator().setTiPozaCalka(poza);
    }
}
/*void MainWindow::on_spinBoxA1_editingFinished() {
    if (uklad) {
        auto wspA = uklad->getModel().getWspA();
        wspA[0] = ui->spinBoxA1->value();
        uklad->getModel().setWspA(wspA);
    }
}

void MainWindow::on_spinBoxA2_editingFinished() {
    if (uklad) {
        auto wspA = uklad->getModel().getWspA();
        wspA[1] = ui->spinBoxA2->value();
        uklad->getModel().setWspA(wspA);
    }
}

void MainWindow::on_spinBoxA3_editingFinished() {
    if (uklad) {
        auto wspA = uklad->getModel().getWspA();
        wspA[2] = ui->spinBoxA3->value();
        uklad->getModel().setWspA(wspA);
    }
}

void MainWindow::on_spinBoxB1_editingFinished() {
    if (uklad) {
        auto wspB = uklad->getModel().getWspB();
        wspB[0] = ui->spinBoxB1->value();
        uklad->getModel().setWspB(wspB);
    }
}

void MainWindow::on_spinBoxB2_editingFinished() {
    if (uklad) {
        auto wspB = uklad->getModel().getWspB();
        wspB[1] = ui->spinBoxB2->value();
        uklad->getModel().setWspB(wspB);
    }
}

void MainWindow::on_spinBoxB3_editingFinished() {
    if (uklad) {
        auto wspB = uklad->getModel().getWspB();
        wspB[2] = ui->spinBoxB3->value();
        uklad->getModel().setWspB(wspB);
    }
}*/
...
/*void MainWindow::on_checkBoxZaklocenie_stateChanged() {
    if (uklad){
        uklad->getModel().wlaczZaklocenie(ui->checkBoxZaklocenie->isChecked());
    }
}

void MainWindow::on_doubleSpinBoxZaklocenie_editingFinished() {
    if (uklad){
        uklad->getModel().ustawPoziomZaklocenia(ui->doubleSpinBoxZaklocenie->value());
    }
}*/
